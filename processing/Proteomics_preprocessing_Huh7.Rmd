...
samples$condition <- c("R1_Huh7_U", "R1_Huh7_T", "R1_Huh7_U", "R1_Huh7_T","R1_Huh7_U", "R1_Huh7_T")
...
```{r}
#normalyzer(jobName=name_ana, designPath=path_design, dataPath=path_data, outputDir=outDir, requireReplicates = FALSE)
```
```{r}
...
```{r}
list_comp <- c("R1_Huh7_T-R1_Huh7_U")
```
...
```
```{r}
pdf("results/virus_levels/Huh7_virus_levels_difference.pdf")
a<- ggplot(data_virus_2, aes(x=Proteins, y=value, fill = "#1F78B4"))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_manual(values =c("#FF7F00"))+ theme_minimal()+ 
...
#FDBF6F
```{r}
pdf("results/virus_levels/Huh7_virus_levels.pdf", height = 5)
a<- ggplot(data_virus, aes(x=variable, y=value, fill = condition))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_manual(values =c("#FF7F00", "#FDBF6F"))+ 
...
```
```{r}
pdf("results/virus_levels/Huh7_virus_levels_boxplot.pdf", height = 5)
a<- ggplot(data_virus, aes(x=variable, y=value, fill = condition))+
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_manual(values =c("#FF7F00", "#FDBF6F"))+ 
  theme(legend.title=element_text(size=8),legend.text=element_text(size=12),
